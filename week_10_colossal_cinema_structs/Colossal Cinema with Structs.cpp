...
*                           find rate function
*
*   2020-12-4 Randi Bolt    Added Structs and adjusted code accordingly
*
* Sources:
...
int main()
{
    ...
    double cost_so_far = 0.00;         //    initialized prior to the "cost per group" loop.
    double cost_per_group = 0.0;       //    This variable increases with each person in the group.
    ...
    nRates = loadRates(RATE_FILE_NAME,rates, MAX_RATES);
        ...
        // if no rates were returned, output error message to user
    if ( nRates <= 0 )
    {
        cout << "unable to load rates from " << RATE_FILE_NAME << endl;
        return BAD_INPUT_FILE;  // end with error code
    }
        // if verbose and rates are loaded tell the user how many rates were loaded from the "filename"
    if ( verbose )
...
int loadRates( string filename, priceTier rates[], int maxRanges )
{
    ...
    cout << "retrieved " << nRates << " rates from " << filename << endl;
    ifile.close();
...
void showRates( priceTier rates[], int nRates )
{
    ...
    {
        cout << fixed << setprecision(2);
        cout << setw(7) << rates[i].lower << setw(9) << rates[i].upper << setw(8) << rates[i].price << "  " << rates[i].rateName << endl;
    } // for
    cout << endl;
} // showRates
...
double findRate( priceTier rates[], int nRates, double age )
{
    ...
    {   // if the age is at least the lower end of this range ...
        ...
        if (age < rates[i].upper and age >= rates[i].lower)
        { // then announce the name of this range and remember the cost of this ticket
            cout << "Age " << age << " gets a " << rates[i].rateName << " ticket." << endl;
            cost = rates[i].price;
        }
